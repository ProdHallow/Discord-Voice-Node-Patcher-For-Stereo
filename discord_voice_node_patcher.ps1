...
public:
    ...
    bool PatchFile() {
        ...
        if (!ApplyPatches(fileData)) { VirtualFree(fileData, 0, MEM_RELEASE); CloseHandle(file); return false; }
    ...
    try {
        switch ($Compiler.Type) {
            'MSVC' {
                ...
                $batContent = "@echo off`ncall `"$vcvars`"`nif errorlevel 1 (`n    echo ERROR: Failed to initialize Visual Studio environment`n    exit /b 1`n)`ncl.exe /EHsc /O2 /std:c++17 ^`n    `"$src1`" ^`n    `"$src2`" ^`n    /Fe`"$exe`" ^`n    /link Psapi.lib`n"
                ...
                if (-not (Test-Path $exe)) { Write-Host "=== Build Log ===" -ForegroundColor Yellow; Write-Host $output }
            }
            'MinGW' {
